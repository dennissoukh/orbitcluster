...
describe('generateIdempotencyKey', () => {
    it('checking for valid prefix', () => {
        expect(generateIdempotencyKey('43a', null)).to.be.a('string').and.satisfy((str) => { return str.startsWith('43a'); });
    });
    ...
    it('checking if specified prefix & length is correct', () => {
        expect(generateIdempotencyKey('43a', 16)).to.have.lengthOf(19).to.be.a('string').and.satisfy((str) => { return str.startsWith('43a'); });
    });
});
...
describe('generateBasePaginationMetadata', () => {
    it('checking for valid query', () => {
        expect(generateBasePaginationMetadata(2, 4, 4, 5, 6)).to.deep.equal({
            page: 2, limit: 4, pages: 1, count: 4, skip: 5, pageCount: 6,
        });
    });
});
